...
function App() {
  ...
  const handleForceSync = async () => {
      if (!isOnline) { alert("No hay conexión a internet. Busca cobertura."); return; }
      setIsSyncing(true);
  ...
  const handleFormSubmit = async (data: Partial<OperationLog>) => {
    ...
    try {
      const logData: Omit<OperationLog, 'id'> = { date: selectedDate, workerId: currentUser.id, machineId: selectedContext.machine.id, type: selectedAction, hoursAtExecution: data.hoursAtExecution || selectedContext.machine.currentHours, ...data };
      await saveOperationLog(logData);
      const newHours = logData.hoursAtExecution && logData.hoursAtExecution > selectedContext.machine.currentHours ? logData.hoursAtExecution : selectedContext.machine.currentHours;
      try {
      ...
      setTimeout(() => { setSuccessMsg(''); setViewState(ViewState.ACTION_MENU); setSelectedAction(null); }, 2000);
    } catch (e) {}
  };
  ...
  if (viewState === ViewState.LOGIN) {
      return (
        <div className="flex flex-col min-h-screen">
            {!isConfigured && (
                <div className="bg-red-600 text-white text-xs text-center p-3 font-bold flex flex-col items-center justify-center gap-1 shadow-md">
                    <div className="flex items-center gap-2"><AlertTriangle size={16} className="text-yellow-300"/> <span>ERROR DE CONFIGURACIÓN</span></div>
                    <span className="font-normal opacity-90 text-[10px] mt-1 bg-red-800 px-2 py-1 rounded">SOLUCIÓN: Ve a Vercel Deployments y haz "Redeploy".</span>
                </div>
            )}
        ...
        <header className="bg-slate-800 text-white shadow-lg sticky top-0 z-20">
          {(!isOnline || pendingItems > 0) && (
              <div className={`text-white text-xs text-center p-2 font-bold flex items-center justify-between px-4 ${isOnline ? 'bg-orange-500' : 'bg-red-600'}`}>
                  <div className="flex items-center gap-2">{!isOnline ? <WifiOff size={14} /> : <DatabaseZap size={14} />} <span>{!isOnline ? 'Sin Conexión' : 'Pendientes'}: {pendingItems}</span></div>
                  {isOnline && pendingItems > 0 && <button onClick={handleForceSync} disabled={isSyncing} className="bg-white/20 px-2 py-1 rounded flex items-center gap-1"><RefreshCcw size={12} className={isSyncing ? 'animate-spin' : ''} /> {isSyncing ? '...' : 'Sincronizar'}</button>}
              </div>
          )}

          <div className="p-4 flex justify-between items-center relative">
            <h1 className="font-bold text-lg flex items-center gap-2 text-red-500"><LayoutDashboard className="w-5 h-5" /> Aridos Marraque</h1>
            <div className="flex items-center gap-3">
                <div className="text-right hidden sm:block"><p className="text-xs text-slate-300">Usuario</p><p className="text-sm font-semibold">{currentUser?.name}</p></div>
                {isUserAdmin ? (
                    <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-1 hover:bg-slate-700 rounded transition-colors">{isMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}</button>
                ) : (
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-md text-center">
                      ...
                      <form 
                        onSubmit={(e) => { 
                          ...
                          const h = Number(formData.get('hours')); 
                          const l = Number(formData.get('litres')); 
